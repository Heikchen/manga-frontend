...
React.useEffect(()=>{fetchMangas()},[page])
    ...
    return(<div className="min-h-4">
        ...
        <div className="flex gap-4 flex-wrap justify-center ">
            {mangas.map((topmangasObject:any)=>(<div className="group w-48 relative h-72 shrink-0"><img className="h-72 m-auto" src={topmangasObject.node.main_picture.medium}/><h1 className="invisible group-hover:visible absolute bottom-6 w-full text-primary-white-color flex items-center justify-center min-h-12 text-2xl bg-primary-red-color text-center">{topmangasObject.node.title}</h1></div>))}
        </div>
